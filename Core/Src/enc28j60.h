...
#define ENC28J60_CS_PIN                                         GPIO_PIN_10

#define ENC28J60_RESET_PORT                                     GPIOC
#define ENC28J60_RESET_PIN                                      GPIO_PIN_11
#define ENC28J60_INT_PORT                                       GPIOC
#define ENC28J60_INT_PIN                                        GPIO_PIN_12
...
// Per-packet control byte bits
#define PKTCTRL_POVERRIDE   (1 << 0)  // Override MACON3
#define PKTCTRL_PCRCEN      (1 << 1)  // Append CRC
#define PKTCTRL_PPADEN      (1 << 2)  // Pad short frames
#define PKTCTRL_PHUGEEN     (1 << 3)  // Allow huge frame

// Common presets
#define PKTCTRL_DEFAULT     (PKTCTRL_POVERRIDE | PKTCTRL_PCRCEN | PKTCTRL_PPADEN)
// → Override MACON3, pad short frames, append CRC

#define PKTCTRL_NOCRC       (PKTCTRL_POVERRIDE | PKTCTRL_PPADEN)
// → Override MACON3, pad short frames, no CRC

#define PKTCTRL_RAW         (PKTCTRL_POVERRIDE)
// → Override MACON3, no pad, no CRC
...
// ENC28J60 Ethernet Interrupt Enable Register (EIE)
#define EIE_INTIE   0x80  // bit 7: Global INT enable
#define EIE_PKTIE   0x40  // bit 6: Receive packet pending interrupt enable
#define EIE_DMAIE   0x20  // bit 5: DMA interrupt enable
#define EIE_LINKIE  0x10  // bit 4: Link status change interrupt enable
#define EIE_TXIE    0x08  // bit 3: Transmit interrupt enable
// bit 2 reserved, must be kept 0
#define EIE_TXERIE  0x02  // bit 1: Transmit error interrupt enable
#define EIE_RXERIE  0x01  // bit 0: Receive error interrupt enable
...
typedef struct {
    // ENC28J60 RX header
    uint16_t nextPtr;        // Next packet pointer
    uint16_t length;         // Frame length
    uint32_t status;         // Receive Status Vector (RSV)
    ...
    // ARP header (if EtherType == 0x0806)
    struct {
        uint16_t hwType;
        uint16_t protoType;
        uint8_t hwSize;
    ...
    // IPv4 header (if EtherType == 0x0800)
    struct {
        uint8_t version_ihl;
        uint8_t tos;
        ...
        uint8_t ttl;
        uint8_t protocol;    // 0x11=UDP, 0x06=TCP, 0x01=ICMP
        uint16_t headerChecksum;
        uint8_t srcIP[4];
        uint8_t dstIP[4];
    } ip;

    // UDP header (if protocol == 0x11)
    struct {
        uint16_t srcPort;
        uint16_t dstPort;
        uint16_t length;
